...
@section('title', 'Editar post - Blog-LCKM')
        ...
        <div class="card-body">
            <form action="{{route('admin.posts.update', $post)}}" method="post" enctype="multipart/form-data" autocomplete="off">
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" class="form-control" id="name_posts" name="name" value="{{$post->name}}" placeholder="Ingrese el Nombre del post">
                ...
                <div class="form-group mb-3">
                    ...
                    <input readonly type="text" class="form-control" id="slug_posts" name="slug" value="{{$post->slug}}" placeholder="Ingrese el Slug del post">
                    @error('slug')
                    <span class="text-danger">{{$message}}</span>
                    @enderror
                </div>
                ...
                <div class="form-group mb-3">
                    ...
                    @foreach($tags as $tag)
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="tags[]" value="{{$tag->id}}" @if(in_array($tag->id, $resul_tags)) checked @endif id="posts-tags">
                ...
                <div class="form-group mb-3">
                    ...
                    @if($post == $post->status) checked @else @endif
                    <div class="form-check">
                        <input class="form-check-input" checked value="1" type="radio" name="status" @if($post->status == 1) checked @endif id="stadus_posts_delete">
                        <label class="form-check-label" for="stadus_posts_delete">
                            Borrador
                        </label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" value="2" type="radio" name="status" @if($post->status == 2) checked @else @endif id="stadus_posts_public">
                ...
                <div class="form-group mb-3">
                    ...
                    <textarea class="form-control" name="extract" id="posts_extract" cols="30" rows="10">{{$post->extract}}</textarea>
                    @error('extract')
                    <span class="text-danger">{{$message}}</span>
                    @enderror
                </div>
                <div class="form-group mb-3">
                    ...
                    <textarea class="form-control" name="body" id="posts_body" cols="30" rows="10">{{$post->body}}</textarea>
                    @error('body')
                    <span class="text-danger">{{$message}}</span>
                    @enderror
                </div>
